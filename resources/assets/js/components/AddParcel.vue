<template>
    <form action="" @submit.prevent="add" :class="{'loading-box': loading}">
        <spinner></spinner>
        <div class="input-group" :class="{'error': errorFields.tracking_number}">
            <label for="tracking_number">{{'tracking_number' | translate}}</label>
            <input v-model="mdl.tracking_number" @blur="errorFields.tracking_number = false" :title="errorFields.tracking_number" id="tracking_number" :placeholder="lang('ex.')+': 42000000123456789'">
        ...
        <div class="input-group" :class="{'error': errorFields.obtain_cost}">
            ...
            <input v-model="mdl.obtain_cost" @blur="errorFields.obtain_cost = false" :title="errorFields.obtain_cost" id="obtain_cost" :placeholder="'price'|translate">
        ...
        <div class="input-group" :class="{'error': errorFields.obtain_webstore}">
            ...
            <input v-model="mdl.obtain_webstore" @blur="errorFields.obtain_webstore = false" :title="errorFields.obtain_webstore" id="obtain_webstore" :placeholder="lang('ex.') + ': www.amazon.com'">
        </div>
        ...
        <div class="input-group">
            ...
            <textarea v-model="mdl.comment" @blur="errorFields.comment = false" :title="errorFields.comment" id="comment" rows="4"></textarea>
        </div>

        <div class="input-group inline">
            <label class="green-text">{{'additional_services' | translate}}: </label>
            <label class="checkbox-wrapper" v-if="toCountry.code === 'GE'">{{'home_delivery' | translate}} <input v-model="mdl.home_delivery" :true-value="1" :false-value="0" type="checkbox"></label>
        </div>
        ...
        <div class="popup-footer">
            <div class="btn-group length-2">
                <a href="#" class="btn white-bg">
                    <label style="display: block;"><input type="file" @change="fileChange($event, 'invoice')" accept="image/*|pdf" style="display: none;">
...
export default {
    ...
    computed: {
        _cats() {
            return this.mdl.order_category_ids.length ? this.mdl.order_category_ids.map((c) => {return _(this.categories).find({id: c}).name;}).join(", ") : this.lang("select_categories");
        },
        toCountries() {
    ...
    methods: {
        setFromCountry(c) {
            ...
            _(this.toCountries).some({id: this.mdl.to_country_id}) || (this.mdl.to_country_id = '');
        },
        setToCountry(c) {
